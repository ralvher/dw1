	...
	<div class="merca-menu col-xs-12 col-lg-12 col-sm-12">
		<div class="col-sm-7 col-lg-7 col-xs-12">
			<ul class="pagination pag"> 
				...
				<li class="active"><a href="semillas.php"  title="Estás en Semillas">Semillas<span class="sr-only">(current)</span></a></li>  
			</ul>
		</div>

		<div class="orden col-sm-4 col-lg-4 col-xs-12">
			<form role="form" method="POST" name="clas" action="semillas.php" >
...
<?php
	...
	if(isset($_POST['ordenar'])){
		if($_POST['ordenar'] == '1'){		
			$consulta = $db->query("SELECT * FROM productos WHERE tipo LIKE '%sem%' ORDER BY nombre ASC");
		}
		elseif($_POST['ordenar'] == '2'){		
			$consulta = $db->query("SELECT * FROM productos WHERE tipo LIKE '%sem%' ORDER BY nombre DESC");
		}
		elseif($_POST['ordenar'] == '3'){		
			$consulta = $db->query("SELECT * FROM productos WHERE tipo LIKE '%sem%' ORDER BY precio ASC");
		}
		elseif($_POST['ordenar'] == '4'){		
			$consulta = $db->query("SELECT * FROM productos WHERE tipo LIKE '%sem%' ORDER BY precio DESC");
		}
	}
	else{
		$consulta = $db->query("SELECT * FROM productos WHERE tipo LIKE '%sem%'");
	}
